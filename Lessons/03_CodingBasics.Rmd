...
vector1[3] # returns 3rd element from vector

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # lists values in order by row rather than by column
matrix2
...
matrix1[4, ] # returns row 4
matrix1[ , 3] # returns column 3
matrix1[c(12, 14)] # returns 12th and 14th value
matrix1[c(12:14)] # returns 12th through 14th value
matrix1[2:4, 1:3] # returns row 2 through 4 and 1 through 3
...
> Answer: Vectors appear as "Values" and lists, matrices, and data frames appear as "Data"

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: You can call up a specific column of the df by name
...
```{r}
high.temp <- c(62, 45, 50, 50, 40, 40, 45, 52, 54, 58)
low.temp <- c(31, 29, 43, 24, 23, 23, 26, 30, 42, 41)
```

Now, create two additional vectors that include the ten-day forecast for the high and low temperatures in Celsius.

```{r}
high.temp.c <- (high.temp -32) * 9/5
low.temp.c <- (low.temp -32) * 9/5
```

Combine your four vectors into a data frame and add informative column names.

```{r}
forecast <- data.frame(high.temp, low.temp, high.temp.c, low.temp.c, 
                              row.names = list("Today", "Friday", "Saturday", "Sunday",
                                            "Monday", "Tuesday", "Wednesday", "Thursday",
                                            "Next Friday", "Next Saturday"))
names(forecast) <- list("High.F", "Low.F", "High.C", "Low.C")
```
