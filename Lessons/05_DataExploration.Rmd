...
# setting limits on x-axis (0 to 500)
...
## can name plot then use syntax print(plot) to have it shown in knitted .rmd doc
```
...
# separating color of frequency polygon by class of whether data USGS approved
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# disadvantage of boxplots, doesn't show relative difference in sample sizes well
ggplot(USGS.flow.data) +
...
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75), scale = "area")
```
