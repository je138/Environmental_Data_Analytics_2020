...
> Add notes here: Those factors may influence primary productivity/how much life the lake supports. May reflect long-term climactic changes.
...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #specifies current format that the dates are in
```
...
`==` #equals (used for factor levels or numeric values)
`!=` # does not equal
`<` # is less than
...
# matrix filtering (must specify rows or columns) (i.e. must have comma)
...
## Tibble is analogous to data frame
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # comma used as an AND operator in this case
...
NTL.phys.data.T1990to1990 <- NTL.phys.data %>% filter(lakename=="Tuesday Lake" 
                                                      & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: More tedious to find all rows with correct year, plus more reproducible to filter by attribute (in case original data set changes, etc.)
...
NTL.phys.data.temp.descending <- NTL.phys.data %>% arrange(desc(temperature_C))
...
# can specify which columns not to select
```
### Mutate
