...
```
## Need to load plyr first, then tidyverse (which contains dplyr, which will mask some commands from plyr as they use the same commands)
...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) # can also attach to the end of the pipe where we filtered
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments)
class(NTL.nutrient.data.PeterPaul$sampledate)
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y") # specifies the format in which the date comes/is currently in

NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% #
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth) # drop any NAs for the depth column
...
NTL.nutrient.data.PeterPaul.subset2 <- NTL.nutrient.data %>% select(-lakeid, -depth_id, -comments)
```
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4) # need to supply what new columns will be
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) # removing NAs in concentration column
...
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate, c("Y", "m", "d")) ## separates y-m-d format and creates new column for each one
...
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) ## joins "horizontally"
...
LitterFiles ## * means "something"

Litter <- LitterFiles %>%
  ldply(read.csv) #used for
```
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), # summarise function from dplyr
