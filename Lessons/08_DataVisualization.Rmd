...
print(O3plot) # use print so it will consistently show up in knit .rmd

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") # color must be listed outside aes()
print(O3plot2)
...
ggplot(subset(PeterPaul.chem.nutrients, depth==0)) +
  geom_point(aes(x=daynum, y=temperature_C, color = year4))+ facet_wrap(vars(lakename), nrow=2)
...
print(NvsP)
## geom_smooth --> line of best fit, in this case linear model (lm). Can remove standard error from line # of best fit using se=FALSE
...
Nutrientplot6 <-
  ...
  geom_density_ridges(alpha = 0.5) + facet_wrap(var(lakename), nrow=2) #
print(Nutrientplot6)
