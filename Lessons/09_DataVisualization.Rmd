...
PeterPaul.chem.nutrients.gathered$sampledate <- as.Date(
  PeterPaul.chem.nutrients.gathered$sampledate, format = "%Y-%m-%d") # must set as date each time you read in the data
```
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + ### specifying where to place the labels
...
NvsP2 <-
  ...
  labs(y= "TN (", mu,"/L)", x="TP (",mu,"/L)") + # change your legend labels here
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x="Lake Name", y=expression(paste("Concentration (",mu,"/L)")), color="Nutrient") +
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(palette = "YlGnBu") +
  #scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84")) +
  scale_color_viridis(discrete = TRUE, end = 0.8, c("nh34","no23","po4",expression(paste("tn_",mu"g")),expression(paste("tp_",mu,
  "g"))) +
  theme(legend.position = "right")
print(Nutrientplot)
```
