...
EPAair.subsample <- sample_n(EPAair, 5000) # shapiro-wilks test can only take up to 5000 samples; taking subsample

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) ## p<.05 means we reject H0 = normal distribution. can't assume normality
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() ## right skewed: common in environmental data w/ concentrations, etc.
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) ## compares data to normal distribution

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") ## if don't specify alternative, will perform two-sided test
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(stat = "count", fill = "gray") +
  #geom_density(fill = "gray") + ## density is essentially a moving average of frequency
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) ## specifies that plot will start at origin
print(Ozone.plot)
```
...
> We are testing whether ozone levels are below the threshold for a "good" AQI index, or in other words whether the population mean ozone level is below 50. Our null hypothesis is that the population mean ozone level is equal to or greater than 50. Based on a p-value of less than 0.05, we reject the null hypothesis and can conclude with relative certainty that the ozone levels are below the threshold for a "good" AQI index. In fact, we are 95% certain that the population mean ozone level is below 41.13.
# Kateri's example: EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threshold for "good" air quality (t-test; t= -57.98, p< 0.0001)
...
```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) # not well illustrated by normal distribution
shapiro.test(EPAair$Ozone[EPAair$Year == 2019])
var.test(EPAair$Ozone ~ EPAair$Year) # tilda = "by"; rejection of null hypothesis implies our assumption of equal variance is not met

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) ## since do not specify alt. hypothesis, will run 2-tailed test
O3.twosample
...
## can look at coefficient on year variable to verify difference in avg ozone based on t-test above
plot(O3.twosample2)
par(mfrow = c(2,2)) ## changes the way R displays plots
```
...
ggplot(data = EPAair, aes(y= Ozone, x= as.factor(Year))) +
  geom_boxplot() +
  labs(caption = O3.twosample$p.value) +
...
> Based on the density plot by year, we can see that 2019 has a higher frequency of ozone readings around and below 50. Conversely, 2018 has a higher frequency of relatively low ozone readings and ozone readings above 50. Based on the boxplots, it seems that 2019 has a slightly higher median ozone value, and both years have IQRs that are entirely the threshold of 50. Bas
