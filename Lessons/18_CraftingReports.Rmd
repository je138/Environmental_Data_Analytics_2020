...
NTL.LTER <- read.csv("~/Courses/EnvDataAnalytics/Environmental_Data_Analytics_2020/Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
```
...
```{r}
library(dplyr)
NTL.new <- NTL.LTER %>% select(-c(lakeid, depth_id, comments)) %>% filter(depth == 0)
...
```{r}
library(tidyr)
summary <- drop_na(NTL.new)
summary <- summary %>% group_by(lakename) %>% summarise(min = min(tn_ug), max = max(tn_ug), mean = mean(tn_ug), sd = sd(tn_ug))
