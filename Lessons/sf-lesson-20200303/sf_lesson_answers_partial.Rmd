...
fl_state_bounds_geom <- southeast_state_bounds %>%
  filter(NAME == "Florida") %>%
  st_geometry()
  # Filters only florida and drops tabular data
...
# bonus
# add your code here
fl_area <- st_area(fl_state_bounds_geom) # find the area of NC
fl_centroid <- st_centroid(fl_state_bounds_geom) # find the centroid of the NC
fl_gages_buffer_100m <- st_buffer(fl_gages, dist = 100) # make a buffer around the stream gages in NC
not_fl_gages <- st_difference(gages, fl_state_bounds_geom) # find stream gages not in NC
```
